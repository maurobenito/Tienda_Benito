...
namespace Tienda_Benito.Controllers
{
    ...
    public class VentaController : Controller
    {
        ...
        {
            // Detección de ordenamiento descendente (convención tipo: "fecha_desc")
        if (!string.IsNullOrEmpty(orderBy) && orderBy.EndsWith("_desc"))
        {
            orderBy = orderBy[..^5];
            desc = true;
        }
    ...
    query = (orderBy?.ToLower(), desc) switch
    {
        ...
        ("fecha", false) => query.OrderByDescending(v => v.Fecha), // siempre descendente por defecto

        _ => query.OrderByDescending(v => v.Fecha) // por defecto
    };
           ...
           foreach (var detalle in venta.Ventadetalles)
{
    ...
    {
        // Producto hijo fraccionado: devolver stock al padre
    ...
    {
        // Producto padre: al devolver stock, recalcular hijos
        var hijos = _context.Producto
            .Where(p => p.ProductoPadreId == detalle.Producto.ProductoId && p.EquivalenciaEnPadre.HasValue)
            .ToList();

        foreach (var hijo in hijos)
        {
            hijo.Stock = (int)(detalle.Producto.Stock / hijo.EquivalenciaEnPadre.Value);
            _context.Producto.Update(hijo);
        }
    }
}
