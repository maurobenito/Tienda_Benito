...
namespace Tienda_Benito.Controllers
{
    ...
    public class ProveedorController : Controller
    {
       ...
       public IActionResult Index(string filtro = "", string orderBy = "Nombre", int pagina = 1, int tamPagina = 5)
{
    ...
    // Detectar campo y dirección (asc/desc)
...
public IActionResult Create(Proveedor proveedor, IFormFile? Archivo1File, IFormFile? Archivo2File)
{
...
public IActionResult Edit(int id, Proveedor proveedor, IFormFile? Archivo1File, IFormFile? Archivo2File)
{
    ...
    if (Archivo1File != null && Archivo1File.Length > 0)
    {
        ...
        if (!string.IsNullOrEmpty(existente.Archivo1))
        {
            var rutaAnterior = Path.Combine(uploads, existente.Archivo1);
    ...
    if (Archivo2File != null && Archivo2File.Length > 0)
    {
        ...
        // Borrar archivo anterior si existe
        if (!string.IsNullOrEmpty(existente.Archivo2))
        {
            var rutaAnterior = Path.Combine(uploads, existente.Archivo2);
            if (System.IO.File.Exists(rutaAnterior))
                System.IO.File.Delete(rutaAnterior);
        }
